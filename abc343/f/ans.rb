...
op = proc { |a, b|
  ...
  h = {}
  top_2 = (ha.keys + hb.keys).uniq.sort.reverse.take(2)
  # p top_2
  top_2.each do |k|
    h[k] = (ha[k] || 0) + (hb[k] || 0)
  end
...
mapping = proc { |f, x| x } # f じゃねえの？
composition = proc { |f, g| g }
seg = LazySegtree.new(A, nil, id, op, mapping, composition)

show = ->() {
  puts N.times.map { |i| seg[i] }.join(" ")
}
